...
const BookingManagement = () => {
  ...
  return <div className="space-y-6">
        ...
        <CardContent className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            <div className="relative">
              ...
              <Input placeholder="Buscar por nombre, email, teléfono o clase..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="pl-10" />
            </div>
            ...
            <Select value={classFilter} onValueChange={setClassFilter}>
              ...
              <SelectContent>
                ...
                {uniqueClasses.map(cls => <SelectItem key={cls.id} value={cls.id}>
                    {cls.title} ({cls.time})
                  </SelectItem>)}
              </SelectContent>
            </Select>
      ...
      <div className="space-y-6">
        {groupedByDay.map(day => <Card key={day.date} className="overflow-hidden">
            ...
            <CardContent className="p-0">
              ...
              <div className="space-y-0">
                {day.classes.map((classGroup, index) => <div key={classGroup.class.id} className={index > 0 ? "border-t" : ""}>
                    {/* Class Header */}
                    ...
                    <Table>
                      ...
                      <TableBody>
                        {classGroup.bookings.map(booking => <TableRow key={booking.id} className={booking.attended === false ? 'bg-red-50/50 hover:bg-red-100/50' : ''}>
                            ...
                            <TableCell>
                              <Badge variant={booking.user_monthly_classes?.remaining_classes === 0 ? "destructive" : "secondary"} className="flex items-center gap-1 w-fit text-xs">
                                {booking.user_monthly_classes?.remaining_classes === 0 && <AlertTriangle className="h-3 w-3" />}
                            ...
                            <TableCell>
                              <div className="flex items-center gap-1">
                                {booking.attended !== true && <Button size="sm" variant="ghost" onClick={() => updateAttendance(booking.id, true)} className="text-green-600 hover:text-green-700 hover:bg-green-100 h-8 px-2">
                                    <CheckCircle className="h-4 w-4" />
                                  </Button>}
                                {booking.attended !== false && <Button size="sm" variant="ghost" onClick={() => updateAttendance(booking.id, false)} className="text-red-600 hover:text-red-700 hover:bg-red-100 h-8 px-2">
